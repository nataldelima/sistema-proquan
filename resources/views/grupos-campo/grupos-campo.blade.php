        ...
        <table class="table table-striped">
            ...
            <tr>
                <td><a style="text-decoration: none"
                        href="{{ route('grupos-campo-show', ['id' => Crypt::encrypt($grupo->id)]) }}">{{ $grupo->nro }}</a>
                </td>
                ...
                <td class="text-end">
                    ...
                    <a href="{{ route('grupos-campo-show', ['id' => Crypt::encrypt($grupo->id)]) }}"
                        class="btn btn-warning mx-2" title="Visualizar informações da congregação"><i class="bi bi-card-list fs-4"></i></a>
                    <a href="{{ route('grupos-campo-edit', ['id' => Crypt::encrypt($grupo->id)]) }}"
                    ...
                    <button type="button" class="btn btn-danger mx-2" data-bs-toggle="modal"
                        data-bs-target="#staticBackdrop{{ $grupo->id }}" data-bs-placement="bottom"
                        title="Apagar registro da congregação"><i class="bi bi-x-circle fs-4"></i>
                    </button>

                    <!-- Modal -->
                    <div class="modal fade" id="staticBackdrop{{ $grupo->id }}" data-bs-backdrop="static"
                        data-bs-keyboard="false" tabindex="-1"
                        aria-labelledby="staticBackdropLabel{{ $grupo->id }}" aria-hidden="true">
